...
function buildTokens(config, siteUrl) {
  ...
  return {
    ...
    HOURS_DISPLAY: config.hours.saturday ? `${config.hours.weekday}, ${config.hours.saturday}` : config.hours.weekday,
    PHONE_SECONDARY_TEL: config.phone.secondaryTel || "",
    PHONE_SECONDARY_DISPLAY: config.phone.secondaryDisplay || "",
...
async function build() {
  ...
  await copyIfExists(imagesDir, join(distDir, "images"));
