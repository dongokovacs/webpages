...
function Feature() {
  ...
  const onMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    setInset(Math.min(Math.max(percentage, 0), 100));
  };
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="pt-12 w-full">
            ...
            >
              ...
              {/* Light image (clipped from left based on slider) */}
              <img
                src={LIGHT_IMAGE}
                alt="Light mode view"
              ...
              {/* Dark image (always behind) */}
              <img
                src={DARK_IMAGE}
                alt="Dark mode view"
                className="absolute left-0 top-0 w-full h-full object-cover rounded-2xl select-none border"
                draggable={false}
              />
            </div>
          </div>
